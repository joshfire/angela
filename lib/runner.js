 ...
 * Load dependencies
 *
 * IMPORTANT: the "trycatch" library must be loaded first, even though it's
 * not used in that module because it wraps global functions such as setTimeout,
 * setImmediate that are used by other libraries ("async" for instance)
 */
require('trycatch');
var fs = require('fs');
...
var argv = optimist
  ...
  .options('before', {
    describe: 'JS file to "require" before tests are run. Use this option to add custom matchers or expose additional variables to the global scope as needed. Code must be synchronous.'
  })
  .options('after', {
    describe: 'JS file to "require" after tests have run. Use this option to run custom code when execution is over. Code must be synchronous.'
  })
  .options('host', {
    default: 'localhost',
  ...
  .options('printerrorstack', {
    boolean: true,
    default: false,
    describe: 'Print stack trace when a test fails because of an exception. This makes failure reports potentially quite verbose but can greatly help you track down the origin of the exception.'
  })
...
var getTestSuites = function (pathname, callback) {
  ...
  // If given path identifies a file, we have our spec file
  var stat = fs.statSync(pathname);
  if (stat.isFile()) {
    suites.push(pathname);
    return callback(suites);
  }
 ...
 */
if (argv.before) {
  argv.before = path.resolve(argv.before);
}
if (argv.after) {
  argv.after = path.resolve(argv.after);
}
