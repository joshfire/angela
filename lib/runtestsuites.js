...
module.exports = function runTestSuites(suites, options, callback) {
  ...
  global.afterEach = jasmineModule.afterEach;
  global.jasmine = jasmineModule.jasmine;
  global.pending = jasmineModule.pending;
  global.By = webdriver.By;
  global.xdescribe = jasmineModule.xdescribe;
  global.xit = jasmineModule.xit;
  global.driver = null;
  ...
  global.describe = function (specDesc, specFunction) {
    return jasmineModule.describe(specDesc, function () {
      if (options.sessionperspec) {
        ...
        jasmineModule.afterEach(function (done) {
        ...
        jasmineModule.afterEach(function () {
          global.driver.controlFlow().removeAllListeners('uncaughtException');
        });
      }
  ...
  global.it = function (description, fn) {
    if (!fn) {
      return jasmineModule.it(description);
    }
    return jasmineModule.it(description, (function (done) {
      global.driver.controlFlow().on('uncaughtException', function (err) {
        jasmineModule.expect(err).toBeNull();
        done();
      });
      if (fn.length) {
        fn(done);
  ...
  var killThemAll = function (cb) {
    process.removeListener('uncaughtException', handleGlobalException);
    if (options.keep) {
  ...
  if (!options.keep) {
    global.jasmine.getEnv().addReporter(new DoneReporter(function () {
