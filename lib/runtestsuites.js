...
/**
 * Patch WebDriverJS control flow execute method to ensure that the function
 * that gets executed inherits the current Node.js domain if one is defined.
 *
 * Internally, domains are used to have tests fail when they throw unexpected
 * exceptions, even if these exceptions are thrown in asynchronous portions of
 * the code. This is done by having "it" callback functions run in a dedicated
 * domain.
 *
 * That is not enough with WebDriver though because it manages its own event
 * loop, started when the WebDriver instance is created and we do create that
 * instance before test suites are run, meaning outside of any domain. WebDriver
 * command functions issued in "it" callback functions run in that event loop
 * and thus do not run in the domain of the "it" callback function.
 *
 * The patch simply makes all WebDriver command functions inherit the domain
 * under which the commands were issued (the domain of the "it" callback
 * function) in practice.
 */
webdriver.promise.ControlFlow.prototype.execute = (function () {
  var execute = webdriver.promise.ControlFlow.prototype.execute;
  return function (fn, desc) {
    if (process.domain) {
      fn = process.domain.bind(fn);
    }
    return execute.call(this, fn, desc);
  };
})();
...
module.exports = function runTestSuites(suites, options, callback) {
  ...
  options.printerrorstack = !!options.printerrorstack;
  ...
  /**
   * Wraps the async test function to report all thrown exception as test
   * failure.
   *
   * The function relies on Node.js domains to track down exceptions in
   * async contexts.
   *
   * @function
   * @param {function} asyncFn The async test function to wrap. The function
   *   must take a "done" function parameter
   * @return {function} An async test function for Jasmine that will report
   *   all errors as test failure.
   */
  var wrapInTryCatch = function (asyncFn) {
    return function (done) {
      trycatch(function () {
        ...
        if (options.printerrorstack && err.stack) {
          jasmineEnv.expect(err.stack).toBeNull();
        }
        else {
  ...
  global.it = function (description, fn) {
    ...
    if (fn.length) {
      // Spec function is asynchronous, ensure we catch all async errors
      return jasmineEnv.it(description, wrapInTryCatch(fn));
    }
    else {
      // Spec function is synchronous
      return jasmineEnv.it(description, fn);
    }
  };
  ...
  var startWebDriverServer = function (cb) {
    ...
    if (options.browser === 'phantomjs') {
      ...
      webdriverServer = childProcess.spawn(
        path.resolve(__dirname, '..', 'drivers', 'chromedriver 4'),
        params
      );
      webdriverServer.stdout.setEncoding('utf-8');
      webdriverServer.stdout.on('data', function (data) {
